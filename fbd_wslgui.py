...
def check_and_install_dependencies():
    ...
    if required_missing_packages:
        ...
        try:
            response = (
                input("\nInstall required dependencies now? [Y/n]: ").strip().lower()
            )
...
def _format_fbdctl_error_payload(error_payload):
    """Return a readable error string from an fbdctl JSON-RPC error payload."""
    if isinstance(error_payload, dict):
        ...
        details = message
        if not details and data is not None:
            details = str(data).strip()
...
def _extract_fbdctl_result(result):
    """Parse subprocess output from fbdctl and normalize success/error handling."""
    ...
    if isinstance(parsed, dict):
        ...
        error_text = _format_fbdctl_error_payload(parsed.get("error"))
    ...
    if not error_text:
        error_text = f"Command failed with exit code {format_process_exit_code(result.returncode)}"

    return False, payload, error_text


def _is_non_retryable_reveal_error(error_text):
    """Return True when a reveal error is deterministic and retries cannot succeed."""
    err = str(error_text or "").lower()
    if not err:
        return False

    non_retryable_tokens = [
        "reveal blind mismatch",
        "malformedcovenant",
        "invalid covenant",
    ]
    return any(token in err for token in non_retryable_tokens)
...
class AuctionMonitor:
    ...
    def _check_all_jobs(self):
        ...
        for job in jobs_data["jobs"]:
            ...
            if not (
                job.get("auto_enabled", False) or job.get("auto_bid_enabled", False)
            ):
                continue
    ...
    def _process_job(self, job, current_height):
        ...
        elif job["status"] == "revealed" and state == "REVEAL" and auto_enabled:
            # Verify reveal is actually confirmed on-chain; self-heal stale/incorrect status.
            # This catches jobs imported as "revealed" before the reveal TX was confirmed,
            # or where the local wallet state diverged from chain state.
            wallet = job.get("wallet")
            bids = (
                self.manager.get_wallet_bids_silent(wallet, job["name"])
                if wallet
                else []
            )
            if bids:  # Only act if we actually got bid data (index-auctions enabled)
                unrevealed = [b for b in bids if not b.get("revealed", False)]
                if unrevealed:
                    self.manager.log(
                        f"[!] '{job['name']}' (wallet '{wallet}'): marked revealed "
                        f"but {len(unrevealed)} bid(s) not confirmed on-chain "
                        f"— re-queuing reveal"
                    )
                    self.manager.update_job_status(
                        job["id"],
                        "bid_placed",
                        message="Re-queued: reveal not confirmed on-chain",
                    )
                    job["status"] = "bid_placed"
                    self._execute_reveal(job)
        ...
        try:
            ...
            if result.get("success"):
                ...
                self.manager.update_job_runtime_fields(
                    job["id"], retry_count=job["retry_count"]
                )
    ...
    def _execute_reveal(self, job):
        ...
        try:
            ...
            name = job["name"]
            self.manager.log(
                f"[*]- Auto-revealing bids for '{name}' (wallet '{wallet}')..."
            )
            ...
            if result.get("success"):
                ...
                self.manager.log(
                    f"[OK] Auto-revealed {len(txids)} bid(s) for '{name}' (wallet '{wallet}')"
                )
                ...
                error = result.get("error", "Unknown error")
                if _is_non_retryable_reveal_error(error):
                    # Attempt auto-repair before giving up.
                    self.manager.log(
                        f"[!] Non-retryable reveal error for '{name}' (wallet '{wallet}'): {error}\n"
                        f"    Attempting automatic repairbid…"
                    )
                    repair = self.manager.execute_repairbid_silent(name, wallet)
                    if repair.get("success"):
                        repaired = repair.get("repaired", 0)
                        self.manager.log(
                            f"[OK] repairbid succeeded for '{name}' (wallet '{wallet}'): "
                            f"{repaired} bid(s) repaired — queuing reveal retry"
                        )
                        ...
                        if reveal_after_repair.get("success"):
                            ...
                            self.manager.log(
                                f"[OK] Auto-revealed {len(txids)} bid(s) for '{name}' "
                                f"(wallet '{wallet}') after repair"
                            )
                            self.manager.notification_manager.notify_revealed(
                                name, job["id"], len(txids)
                            )
                        else:
                            reveal_err = reveal_after_repair.get("error", "Unknown")
                            self.manager.log(
                                f"[!] Reveal still failed after repair for '{name}' "
                                f"(wallet '{wallet}'): {reveal_err} — will retry next poll"
                            )
                    else:
                        repair_err = repair.get("error", "Unknown")
                        self.manager.log(
                            f"[!] repairbid also failed for '{name}' (wallet '{wallet}'): "
                            f"{repair_err} — marking failed"
                        )
                        self.manager.update_job_status(
                            job["id"],
                            "failed",
                            error=f"Reveal failed (non-retryable, repair also failed): {error}",
                        )
                        self.manager.notification_manager.notify_failed(
                            name, job["id"], error
                        )
                ...
                # Retry logic for transient reveal failures
                job["retry_count"] = job.get("retry_count", 0) + 1
                self.manager.update_job_runtime_fields(
                    job["id"], retry_count=job["retry_count"]
                )
                ...
                if job["retry_count"] < 3:
                    self.manager.log(
                        f"[!] Reveal failed for '{name}' (wallet '{wallet}'), will retry (attempt {job['retry_count']}/3): {error}"
                    )
                else:
                    self.manager.update_job_status(
                        job["id"],
                    ...
                    self.manager.log(
                        f"[OK]- Reveal failed for '{name}' (wallet '{wallet}') after 3 retries: {error}"
                    )
                    # Stage 4: Notify failure
                    self.manager.notification_manager.notify_failed(
                        name, job["id"], error
            ...
            self.manager.log(
                f"[OK]- Error executing auto-reveal for '{job['name']}' (wallet '{job.get('wallet', '?')}'): {e}"
            )
            # Stage 4: Notify error
            self.manager.notification_manager.notify_failed(
                job["name"], job["id"], str(e)
...
class FBDManager:
    ...
    def __init__(self, root):
        """Initialize the instance state."""
        self.root = root
        self.ui_toolkit_mode = (
            _normalize_ui_toolkit(getattr(root, "_ui_toolkit_mode", ""))
            or UI_TOOLKIT_TTK
        )
        self.customtkinter_active = (
            self.ui_toolkit_mode == UI_TOOLKIT_CUSTOMTK and ctk is not None
        )
        ...
        self.ui_toolkit_choice_var = tk.StringVar(
            value=self.ui_toolkit_labels.get(
                self.ui_toolkit_mode, self.ui_toolkit_labels[UI_TOOLKIT_TTK]
            )
        )
        ...
        self.custom_dns_port_var = tk.StringVar(
            value=self.config.get("custom_dns_port", "")
        )
        self.miner_download_url_var = tk.StringVar(
            value=self.config.get("miner_download_url", "https://l.woodburn.au/miner")
        )
        ...
        self.auto_restart_var = tk.BooleanVar(
            value=bool(self.config.get("auto_restart", False))
        )
        self.restart_delay_var = tk.StringVar(
            value=str(self.config.get("restart_delay", "3"))
        )
        ...
        self.log("=" * 60)
        self.log("FBD Node Manager GUI v6.0.0 Started")
    ...
    def _create_section_frame(self, parent, text, padding=10):
        ...
        if self.customtkinter_active:
            ...
            title = ctk.CTkLabel(
                ...
                text=text,
                font=ctk.CTkFont(
                    family="Arial", size=10, weight="bold", underline=True
                ),
                anchor="w",
                text_color=title_fg,
            )
    ...
    def _create_button(self, parent, text, command, state="normal", **kwargs):
        ...
        if self.customtkinter_active:
            ...
            button_kwargs.setdefault(
                "border_color", "#7a7a7a" if is_dark else "#9a9a9a"
            )
            button_kwargs.setdefault("text_color", "#f2f2f2" if is_dark else "#111111")
            button_kwargs.setdefault(
                "text_color_disabled", "#8a8a8a" if is_dark else "#6e6e6e"
            )
    ...
    def _create_log_widget(self, parent, lines=10, **kwargs):
        ...
        if self.customtkinter_active:
            ...
            textbox_kwargs.setdefault(
                "border_color", "#4a4a4a" if is_dark else "#bdbdbd"
            )
    ...
    def _enable_button_wrap(
        ...
        """Reflow a button row into multiple lines when the frame gets narrow."""
        if not buttons:
            return
        ...
        def _reflow(_event=None):
            ...
            button_widths = [
                max(btn.winfo_reqwidth(), min_button_width) + (2 * padx)
                for btn in buttons
            ]
            total_required = sum(button_widths)

            if width >= total_required:
                cols = len(buttons)
            else:
                cols = 0
                used = 0
                for btn_width in button_widths:
                    if cols == 0 or used + btn_width <= width:
                        used += btn_width
                        cols += 1
                    else:
                        break
        ...
        parent.bind("<Configure>", _reflow, add="+")
        parent.after_idle(_reflow)

    def _attach_treeview_scrollbars(self, frame, tree, *, fill="both", expand=True):
        """Attach vertical and horizontal scrollbars to a treeview."""
        y_scroll = ttk.Scrollbar(frame, orient="vertical", command=tree.yview)
        x_scroll = ttk.Scrollbar(frame, orient="horizontal", command=tree.xview)
        tree.configure(yscrollcommand=y_scroll.set, xscrollcommand=x_scroll.set)

        # Keep column widths fixed so horizontal scroll appears when columns overflow.
    ...
    def _refresh_ctk_container_theme(self, effective_mode):
        ...
        for entry in self._ctk_entries:
            if entry.winfo_exists():
                entry.configure(
                    fg_color=entry_bg, border_color=entry_border, text_color=entry_fg
                )
    ...
    def apply_theme(self):
        ...
        if self.customtkinter_active:
            ...
            # Keep default color theme stable.
        ...
        style.configure(
            "TLabelframe", background=palette["bg"], bordercolor=palette["line"]
        )
        style.configure(
        ...
        style.map(
            "TEntry",
            fieldbackground=[
                ("readonly", palette["disabled_bg"]),
                ("disabled", palette["disabled_bg"]),
            ],
            foreground=[
                ("readonly", palette["disabled_fg"]),
        ...
        style.map(
            ...
            foreground=[("active", "#111111"), ("disabled", palette["muted_fg"])],
            indicatorbackground=[
                ("selected", palette["checkbox_bg"]),
                ("!selected", palette["checkbox_bg"]),
            ],
        )
        style.configure(
            "TNotebook", background=palette["bg"], bordercolor=palette["line"]
        )
        style.configure(
            "TNotebook.Tab",
            background=palette["section"],
    ...
    def on_ui_toolkit_changed(self, _event=None):
        ...
        selected_mode = self.ui_toolkit_label_to_mode.get(
            selected_label, UI_TOOLKIT_TTK
        )
    ...
    def create_node_tab(self):
        ...
        # Mining config frame
        config_outer, config_frame = self._create_section_frame(
            scrollable_frame, "Mining Configuration"
        )
        ...
        # Pool miner
        pool_outer, pool_frame = self._create_section_frame(
            scrollable_frame, "Pool Miner"
        )
        ...
        self._enable_button_wrap(
            pool_button_frame,
            [self.start_pool_miner_btn, self.stop_pool_miner_btn],
        ...
        mining_stats_btn = self._create_button(
            button_frame, text="Mining Stats", command=self.get_mining_stats
        )
        delete_chain_btn = self._create_button(
        ...
        # Log output
        log_outer, log_frame = self._create_section_frame(
            scrollable_frame, "Node Output", padding=5
        )
        ...
        clear_log_btn = self._create_button(
            log_controls, text="Clear Log Display", command=self.clear_log_display
        )
        open_log_btn = self._create_button(
            log_controls, text="Open Log File", command=self.open_log_file
        )
        self._enable_button_wrap(
            log_controls,
            [clear_log_btn, open_log_btn],
        ...
        log_path_label.grid(
            row=10, column=0, columnspan=3, sticky="w", padx=10, pady=(2, 0)
        )
    ...
    def create_wallet_tab(self):
        ...
        wallet_row = self._create_inline_frame(wallet_frame)
        wallet_row.pack(fill="x")

        ttk.Label(wallet_row, text="Active Wallet:").pack(side="left", padx=5)
        ...
        wallet_buttons = self._create_inline_frame(wallet_frame)
        wallet_buttons.pack(fill="x", pady=(5, 0))

        copy_address_btn = self._create_button(
            wallet_buttons,
            text="Copy Address",
            command=self.copy_wallet_address_dialog,
        )
        create_wallet_btn = self._create_button(
            wallet_buttons, text="Create Wallet", command=self.create_wallet
        )
        import_wallet_btn = self._create_button(
            wallet_buttons, text="Import Wallet", command=self.import_wallet
        )
        delete_wallet_btn = self._create_button(
            wallet_buttons, text="Delete Wallet", command=self.delete_wallet
        )

        self._enable_button_wrap(
            wallet_buttons,
            [copy_address_btn, create_wallet_btn, import_wallet_btn, delete_wallet_btn],
        ...
        # Send payment
        send_outer, send_frame = self._create_section_frame(
            scrollable_frame, "Send Payment", padding=10
        )
        ...
        self._create_entry(
            send_frame, textvariable=self.send_amount_var, width=50
        ...
        self._attach_treeview_scrollbars(tx_tree_frame, self.tx_tree)

        self._make_treeview_sortable(self.tx_tree, tx_columns)
        self.tx_tree.bind("<Button-1>", self._on_tx_tree_click)
        self.tx_tree.bind(
            "<Double-1>", lambda e: self._on_tx_tree_click(e, open_any_column=True)
        )
    ...
    def create_auction_tab(self):
        ...
        self.jobs_tree = ttk.Treeview(
            jobs_tree_frame, columns=columns, show="headings", height=10
        )
        ...
        self._attach_treeview_scrollbars(jobs_tree_frame, self.jobs_tree, fill="both")
        ...
        self.jobs_refresh_label.grid(
            row=10, column=0, columnspan=4, sticky="w", padx=10, pady=(2, 0)
        )
        ...
        self._create_button(
            name_frame, text="Get Name Info", command=self.get_name_info
        ...
        action_frame.grid(row=1, column=0, columnspan=5, sticky="ew", pady=10)

        open_auction_btn = self._create_button(
            action_frame, text="Open Auction", command=self.send_open
        )
        place_bid_btn = self._create_button(
            action_frame, text="Place Bid", command=self.send_bid
        )
        reveal_bid_btn = self._create_button(
            action_frame, text="Reveal Bid", command=self.send_reveal
        )
        repair_bid_btn = self._create_button(
            action_frame, text="Repair Bid", command=self.send_repair_bid
        )
        register_btn = self._create_button(
            action_frame, text="Register", command=self.send_register
        )
        self._enable_button_wrap(
            action_frame,
            [
        ...
        # Bid parameters
        bid_outer, bid_frame = self._create_section_frame(
            scrollable_frame, "Bid Parameters", padding=10
        )
        ...
        self._create_entry(
            bid_frame, textvariable=self.bid_amount_var, width=bid_entry_width
        ...
        self._create_entry(
            bid_frame, textvariable=self.lockup_amount_var, width=bid_entry_width
        ...
        self._enable_button_wrap(
            notif_controls,
            [notif_refresh_btn, notif_clear_btn, notif_mark_btn],
        ...
        # My names
        mynames_outer, mynames_frame = self._create_section_frame(
            scrollable_frame, "My Names", padding=5
        )
        ...
        self._attach_treeview_scrollbars(mynames_frame, self.names_tree)
    ...
    def create_block_calc_tab(self):
        ...
        input_outer, input_frame = self._create_section_frame(
            scrollable_frame, "Input Method", padding=10
        )
        ...
        self._create_entry(
            name_entry_frame, textvariable=self.calc_name_var, width=name_field_width
        ).pack(side="left", padx=5)
        self._create_button(
            name_entry_frame,
        ...
        self._create_entry(
            manual_grid, textvariable=self.calc_open_block_var, width=15
        ).grid(row=0, column=1, pady=2, padx=5)
        ttk.Label(
            manual_grid,
            text="(~1 hour period)",
            font=("Arial", 8, "italic"),
        ...
        self._create_entry(
            manual_grid, textvariable=self.calc_bid_start_var, width=15
        ).grid(row=1, column=1, pady=2, padx=5)
        ttk.Label(
            manual_grid,
        ...
        self._create_entry(
            manual_grid, textvariable=self.calc_reveal_start_var, width=15
        ).grid(row=2, column=1, pady=2, padx=5)
        ttk.Label(
            manual_grid,
        ...
        self._create_entry(
            manual_grid, textvariable=self.calc_closed_block_var, width=15
        ).grid(row=3, column=1, pady=2, padx=5)
        ttk.Label(
            manual_grid,
        ...
        self._attach_treeview_scrollbars(results_frame, self.calc_results_tree)
    ...
    def create_settings_tab(self):
        ...
        load_profile_btn = self._create_button(
            profile_row2, text="Load Profile", command=self.load_profile
        )
        save_new_profile_btn = self._create_button(
            profile_row2, text="Save as New", command=self.save_new_profile
        )
        update_profile_btn = self._create_button(
            profile_row2, text="Update Current", command=self.update_profile
        )
        delete_profile_btn = self._create_button(
        ...
        self._create_entry(
            advanced_frame, textvariable=self.custom_datadir_var, width=40
        ...
        self._create_entry(
            advanced_frame, textvariable=self.custom_dns_port_var, width=40
        ...
        self._create_entry(
            restart_frame, textvariable=self.restart_delay_var, width=10
        ).pack(anchor="w")
        ...
        self._create_entry(
            email_frame, textvariable=self.email_smtp_server_var, width=30
        ...
        self._create_entry(
            email_frame, textvariable=self.email_smtp_port_var, width=30
        ).grid(row=2, column=1, sticky="ew", pady=2)
        ...
        self._create_entry(
            email_frame, textvariable=self.email_from_var, width=30
    ...
    def handle_node_crash(self, exit_code):
        ...
        self.root.after(
            0,
            lambda: self.restart_label.configure(
                text=f"Restarts: {self.restart_count}"
            ),
        )
    ...
    def copy_wallet_address_dialog(self):
        ...
        def copy_selected_wallet_address():
            ...
            result_label.configure(
                text=f"Copied address for wallet '{selected_wallet}'."
            )
        ...
        close_btn = self._create_button(
            button_frame, text="Close", command=close_dialog
        )
    ...
    def create_wallet(self):
        ...
        ttk.Label(dialog, text="Enter wallet name:", font=("Arial", 10)).pack(
            pady=(18, 5)
        )
    ...
    def show_seed_dialog(self, wallet_name, seed_phrase):
        ...
        self._create_button(
            dialog, text="I Have Saved My Seed", command=dialog.destroy
        ).pack(pady=10)
    ...
    def _show_block_calc_popup(self, name, rows):
        ...
        self._create_button(button_frame, text="Close", command=popup.destroy).pack(
            side="right"
        )
    ...
    def show_selected_job_block_calc(self):
        ...
        if not rows:
            self.log(
                f"Block calculator: No popup shown for '{name}' because no rows were available"
            )
            return
    ...
    def refresh_jobs_list(self):
        ...
        try:
            # A manual/auto refresh should also run due automation checks immediately.
            if hasattr(self, "auction_monitor") and self.auction_monitor:
                try:
                    self.log("Jobs refresh: running due automation scan", "debug")
            ...
            for job in jobs:
                ...
                highest_rival_lockup = self._to_float(
                    snapshot.get("highest_rival_lockup", 0.0)
                )
                local_only_lockup = bool(snapshot.get("local_only_lockup", False))
                if highest_rival_lockup > 0:
                    highest_lockup_text = (
                        f"{self._format_fbc(highest_rival_lockup)} FBC"
                    )
    ...
    def _build_job_snapshot(self, job):
                ...
                if isinstance(obj, dict):
                    for key, val in obj.items():
                        ...
                        if (
                            ...
                            or (
                                "highest" in key_l
                                and ("lock" in key_l or "bid" in key_l)
                            )
                        ):
                            _push_value(val)
                        _walk(val, depth + 1)
        ...
        for b in rival_bids:
            ...
            if (
                (not revealed)
                and lockup > original_bid
                and snapshot["state"] == "REVEAL"
            ):
    ...
    def _estimate_name_effective_value(self, wallet, name, fallback_value_fbc):
        ...
        try:
            ...
            if rival_values:
                next_highest = max(
                    [v for v in rival_values if v <= our_bid], default=max(rival_values)
                )
    ...
    def _sync_job_status_from_chain(self, job, snapshot):
        """Promote stale bid_placed jobs to revealed only when all local bids are revealed."""
        ...
        # Do not mark revealed until every bid in this wallet has been revealed.
        unrevealed = [b for b in bids if not b.get("revealed", False)]
        if unrevealed:
            return False
        ...
        try:
            ...
            def _extract_transactions(payload):
                ...
                if isinstance(root, dict):
                    ...
                    if any(
                        k in root for k in ("txid", "hash", "transactionid", "txHash")
                    ):
                        return [root]
                    ...
                    for key in ("transactions", "txs", "history", "items", "list"):
                        ...
                        if isinstance(val, dict):
                            dict_rows = [
                                row for row in val.values() if isinstance(row, dict)
                            ]
                            if dict_rows:
                                return dict_rows
            ...
            if result.returncode != 0:
                messagebox.showerror(
                    "Error", f"Failed to load transactions: {result.stderr}"
                )
                return
    ...
    def _open_transaction_explorer(self, txid):
        ...
        self.log(
            f"Failed to open explorer URL for txid {txid}: no working URL opener found"
        )
    ...
    def send_reveal(self):
        """Reveal bids for a name, with per-bid selection when multiple unrevealed bids exist."""
        wallet = self.wallet_name_var.get()
        name = self.name_var.get()
        ...
        if len(unrevealed) > 1:
            ...
            # Single/no unrevealed bid — reveal all (original behaviour).
            if not messagebox.askyesno("Confirm", f"Reveal all bids for '{name}'?"):
                return
            self._do_reveal_all(wallet, name)

    def _do_reveal_all(self, wallet, name):
        """Send sendreveal for all bids of a name and log the result."""
        try:
    ...
    def _send_reveal_bid_picker(self, wallet, name, unrevealed_bids):
        """Show a dialog listing unrevealed bids so the user can reveal them individually or all at once."""
        dialog = tk.Toplevel(self.root)
        dialog.title(f"Reveal Bids — {name} ({wallet})")
        dialog.resizable(False, False)
        dialog.grab_set()

        ttk.Label(
            dialog,
            text=f"{len(unrevealed_bids)} unrevealed bid(s) found for '{name}' in wallet '{wallet}'.",
            font=("Arial", 10, "bold"),
        ).pack(padx=15, pady=(12, 4))
        ttk.Label(
            dialog,
            text="Select a bid to reveal individually, or reveal all at once.",
            font=("Arial", 9),
        ...
        for bid in unrevealed_bids:
            lockup_fbc = bid.get("lockup", 0) / 1_000_000
            value_fbc = bid.get("value", 0) / 1_000_000
            needs_repair = bid.get("needsRepair", False)
            outpoint = bid.get("outpoint", {})
            tx_hash = outpoint.get("hash", "")[:12]
            label = (
                f"Bid {value_fbc:.2f} / Lockup {lockup_fbc:.2f} FBC  "
                f"[tx:{tx_hash}...]  {'⚠ needsRepair' if needs_repair else ''}"
            )
            bid_value_arg = value_fbc if value_fbc > 0 else None

            row = ttk.Frame(frame)
            row.pack(fill="x", pady=2)
            ttk.Label(row, text=label, width=60, anchor="w").pack(side="left")

            def _reveal_one(w=wallet, n=name, v=bid_value_arg, d=dialog):
                # sendreveal applies to all bids in wallet for name;
                # use --value flag if the node supports it, otherwise fall back to reveal-all.
                d.destroy()
                try:
                    if v is not None:
                        cmd, _ = self.get_fbdctl_command(
                            "--wallet", w, "sendreveal", n, "--value", str(v)
                        )
                    else:
    ...
    def send_repair_bid(self):
        ...
        self._show_repair_bid_dialog(wallet, name)

    def _show_repair_bid_dialog(self, wallet, name, on_success=None):
        """
        Dialog: optionally enter an explicit bid value (FBC), then run repairbid.
        on_success: optional callable(result_dict) invoked after a successful repair.
        """
        # Fetch current bids to show needsRepair status.
        bids = self.get_wallet_bids_silent(wallet, name)
        needs_repair_bids = [b for b in bids if b.get("needsRepair", False)]

        dialog = tk.Toplevel(self.root)
        dialog.title(f"Repair Bid — {name} ({wallet})")
        dialog.resizable(False, False)
        dialog.grab_set()

        ttk.Label(
            dialog,
            text=f"Repair bid data for '{name}' (wallet '{wallet}')",
            font=("Arial", 10, "bold"),
        ).pack(padx=15, pady=(12, 2))

        if needs_repair_bids:
            for b in needs_repair_bids:
                lockup_fbc = b.get("lockup", 0) / 1_000_000
                ttk.Label(
                    dialog,
                    text=f"  ⚠ Bid needing repair: lockup={lockup_fbc:.6f} FBC",
                    foreground="#cc6600",
                ).pack(padx=15, anchor="w")
        else:
            ttk.Label(
                dialog,
                text="  (No bids flagged needsRepair — repair can still be attempted)",
                foreground="#666",
            ).pack(padx=15, anchor="w")
        ...
        ttk.Label(
            dialog,
            text=(
                "Actual bid value (FBC) — leave blank to let node brute-force\n"
                "(tries whole FBC, 0.01 FBC … up to lockup amount)"
            ),
            font=("Arial", 9),
        ...
        value_entry.pack(padx=15, anchor="w")

        status_var = tk.StringVar()
        status_label = ttk.Label(dialog, textvariable=status_var, foreground="blue")
        status_label.pack(padx=15, pady=(6, 0), anchor="w")
        ...
        def do_repair():
            ...
            result = self.execute_repairbid_silent(name, wallet, value_fbc)
            if result["success"]:
                repaired = result.get("repaired", 0)
                msg = f"Repaired {repaired} bid(s) for '{name}'."
                self.log(f"[OK] {msg}")
                status_var.set(f"[OK] {msg}")
                messagebox.showinfo("Repair Successful", msg)
                dialog.destroy()
                if on_success:
                    on_success(result)
            else:
            ...
            if result.returncode == 0:
                ...
                if not names or len(names) == 0:
                    ...
                    for name_data in names:
                        name_info = (
                            name_data.get("name", {})
                            if isinstance(name_data.get("name"), dict)
                            else {}
                        )
                        name_str = name_info.get("string") or (
                            name_data.get("name")
                            if isinstance(name_data.get("name"), str)
                            else "N/A"
                        )
                        value_raw = name_data.get("value", 0) or name_info.get(
                            "value", 0
                        )
                        value_fbc = value_raw / 1_000_000
                        value_fbc = self._estimate_name_effective_value(
                            wallet, name_str, value_fbc
                        )
                        ...
                        self.names_tree.insert(
                            "",
                            "end",
                            values=(name_str, f"{value_fbc:.6f}", renew_due_text),
                        )
    ...
    def _sort_treeview_column(self, tree, col):
        ...
        def _parse_relative_age_seconds(value):
            ...
            return sum(
                float(amount) * multipliers.get(unit, 0.0) for amount, unit in parts
            )
    ...
    def _show_names_dns_dialog(self):
        ...
        if not selected:
            messagebox.showwarning(
                "Warning", "Please select a name from the table first."
            )
            return
    ...
    def _show_dns_manager(self, name):
        ...
        dns_tree = ttk.Treeview(
            tv_frame, columns=("Type", "Data"), show="headings", height=12
        )
        ...
        ttk.Button(
            btn_frame, text="Add", command=lambda: self._dns_add_record(dns_win)
        ).pack(side="left", padx=2)
        ttk.Button(
            btn_frame, text="Edit", command=lambda: self._dns_edit_record(dns_win)
        ).pack(side="left", padx=2)
        ttk.Button(
            btn_frame, text="Remove", command=lambda: self._dns_remove_record(dns_win)
        ).pack(side="left", padx=2)
        ttk.Separator(btn_frame, orient="vertical").pack(side="left", padx=6, fill="y")
        ttk.Button(
            btn_frame,
            text="Save to Chain",
            command=lambda: self._dns_save_to_chain(dns_win),
        ).pack(side="left", padx=2)
        ttk.Button(
            btn_frame, text="Refresh", command=lambda: self._dns_load_records(dns_win)
        ).pack(side="left", padx=2)
        ttk.Button(btn_frame, text="Docs \u2197", command=self._open_dns_docs).pack(
            side="left", padx=2
        )
        ttk.Button(btn_frame, text="Close  [Esc]", command=_on_close).pack(
            side="right", padx=2
        )
        ...
        ttk.Label(
            dns_win,
            textvariable=dns_win.status_var,
            font=("Arial", 8),
            foreground="gray",
    ...
    def _dns_refresh_tree(self, dns_win):
        ...
        for i, rec in enumerate(dns_win.records):
            tree.insert(
                ...
                "end",
                iid=str(i),
                values=(rec.get("type", "?"), self._format_dns_record_summary(rec)),
            )
    ...
    def _format_dns_record_summary(self, rec):
        ...
        if typ == "CAA":
            return (
                f"[{rec.get('flags', 0)}]  {rec.get('tag', '')}  {rec.get('value', '')}"
            )
        if typ == "SUB":
    ...
    def _get_dns_field_specs(self, typ):
        ...
        specs = {
            ...
            "MX": [
                ("Exchange hostname", "exchange", "entry", ""),
                ("Preference  (0=highest priority)", "preference", "entry", "10"),
            ],
            "TXT": [
                ("Text values  (one per line, each \u2264255 bytes)", "txt", "text", [])
            ],
            "DS": [
                ("Key Tag (integer)", "keyTag", "entry", "0"),
                (
    ...
    def _dns_add_record(self, dns_win):
        ...
        def on_save(record):
            ...
            self._dns_refresh_tree(dns_win)
            dns_win.status_var.set(
                f"{len(dns_win.records)} record(s) \u2014 unsaved changes"
            )
    ...
    def _dns_edit_record(self, dns_win):
        ...
        if not selected:
            messagebox.showwarning(
                "Warning", "Select a record to edit.", parent=dns_win
            )
            return
        idx = int(selected[0])
    ...
    def _dns_remove_record(self, dns_win):
        ...
        if not selected:
            messagebox.showwarning(
                "Warning", "Select a record to remove.", parent=dns_win
            )
            return
        idx = int(selected[0])
        if idx >= len(dns_win.records):
    ...
    def _dns_record_edit_dialog(self, dns_win, record=None, on_save=None):
        ...
        def rebuild_fields(*_):
            ...
            for label, key, wtype, default in self._get_dns_field_specs(typ):
                ...
                row.pack(fill="x", pady=2)
                ttk.Label(
                    row, text=label + ":", width=14, wraplength=130, justify="left"
                ).pack(side="left", anchor="n", padx=(0, 4))
                existing = (
                    record.get(key) if (record and record.get("type") == typ) else None
                )
                if wtype == "entry":
                    ...
                    txt_w = scrolledtext.ScrolledText(
                        row, height=4, width=38, wrap=tk.WORD
                    )
                    ...
                    vals_list, def_val = (
                        default if isinstance(default, tuple) else ([], default)
                    )
                ...
                if wtype == "text":
                    ...
                    if key == "records_json":
                        try:
                            ...
                            messagebox.showerror(
                                "Invalid JSON",
                                "Nested records field is not valid JSON.",
                                parent=dialog,
                            )
                            return
                    else:
        ...
        ttk.Button(btn_row, text="Cancel", command=dialog.destroy, width=8).pack(
            side="left"
        )
    ...
    def _dns_save_to_chain(self, dns_win):
        ...
        if not wallet:
            messagebox.showwarning(
                "Warning",
                "No wallet selected. Set active wallet in the Wallet tab.",
                parent=dns_win,
            )
            return
        if not messagebox.askyesno(
        ...
        try:
            ...
            cmd, _ = self.get_fbdctl_command(
                "--wallet", wallet, "sendupdate", dns_win.name, resource_json
            )
            result = subprocess.run(
                cmd,
                capture_output=True,
    ...
    def load_saved_settings(self):
        ...
        toolkit_mode = (
            _normalize_ui_toolkit(self.config.get("ui_toolkit", UI_TOOLKIT_TTK))
            or UI_TOOLKIT_TTK
        )
        ...
        self.ui_toolkit_choice_var.set(
            self.ui_toolkit_labels.get(
                toolkit_mode, self.ui_toolkit_labels[UI_TOOLKIT_TTK]
            )
        )
        ...
        self.theme_choice_var.set(
            self.theme_mode_labels.get(theme_mode, "Same as system")
        )
    ...
    def check_fbd_version(self):
        ...
        threading.Thread(
            target=self._check_fbd_version_thread, args=(url,), daemon=True
        ).start()
    ...
    def download_or_update_fbd(self):
        ...
        threading.Thread(
            target=self._download_or_update_fbd_thread, args=(url,), daemon=True
        ).start()
    ...
    def _download_or_update_fbd_thread(self, url):
        ...
        try:
            ...
            remote_fbd, remote_fbdctl = self._extract_fbd_binaries_from_zip(
    ...
    def startup_binary_update_check(self):
        ...
        threading.Thread(
            target=self._startup_binary_update_check_thread, daemon=True
        ).start()
        ...
        try:
            ...
            if miner_url.startswith("http"):
                ...
                if miner_temp.exists() and miner_temp.stat().st_size > 0:
                    ...
                    local_hash = (
                        self._get_file_sha256(miner_path) if miner_path.exists() else ""
                    )
            ...
            if fbd_url.startswith("http"):
                ...
                with open(zip_path, "wb") as f:
                    for chunk in r.iter_content(chunk_size=8192):
                        if chunk:
                            f.write(chunk)
                remote_fbd, _ = self._extract_fbd_binaries_from_zip(
                    zip_path, extract_dir
                )
                remote_hash = self._get_file_sha256(remote_fbd)
                local_hash = (
                    self._get_file_sha256(fbd_path) if fbd_path.exists() else ""
                )
                if remote_hash != local_hash:
                    updates.append("fbd")
            if zip_path.exists():
    ...
    def verify_transaction_confirmed(self, txid, min_confirmations=1, wallet=None):
        """
        ...
        """
        try:
            tx = self.get_transaction_silent(txid, wallet=wallet)
    ...
    def get_transaction_silent(self, txid, wallet=None):
        """Get transaction details without UI updates; prefer the job wallet when provided."""
        try:
            args = []
            if wallet:
                args.extend(["--wallet", wallet])
            args.extend(["gettransaction", txid])

            cmd, _ = self.get_fbdctl_command(*args)
            result = subprocess.run(
                cmd,
                capture_output=True,
                text=True,
            ...
            ok, data, _err = _extract_fbdctl_result(result)
            if not ok:
                return None
    ...
    def restore_auction_jobs_on_startup(self):
        ...
        if active_jobs:
            ...
            if self.rpc_call("getblockchaininfo") is None:
                self.log(
                    "Startup job verification skipped: node RPC not reachable yet",
                    "debug",
                )
                return
        ...
        try:
            ...
            for tx_type, txid in txids.items():
                if txid and isinstance(txid, str):  # Single txid
                    if not self.verify_transaction_confirmed(txid, wallet=wallet):
                        self.log(
                    ...
                    for t in txid:
                        if not self.verify_transaction_confirmed(t, wallet=wallet):
                            self.log(
            ...
            if not wallet_info:
                self.log(
                    f"[!] Job {job['name']}: wallet '{wallet}' not accessible",
                    "warning",
                )
...
"""
    ...
    def offer_rollout_reminder(self, name, rollout_height):
        ...
        if self.has_rollout_reminder(name, rollout_height):
            existing = [
                r
                for r in self.rollout_reminders
                if r.get("name") == name
        ...
        def refresh_tree():
            ...
            if reminders:
                ...
                for reminder in reminders:
                    reminder_id = (
                        reminder.get("id") or f"row-{len(tree.get_children())}"
                    )
                    tree.insert(
                        "",
                        tk.END,
    ...
    def show_watchlist_manager(self):
        ...
        button_row = ttk.Frame(frame)
        button_row.pack(fill="x", pady=(10, 0))
        self._create_button(button_row, text="Use Name", command=use_name).pack(
            side="left"
        )
    ...
    def execute_repairbid_silent(self, name, wallet, value_fbc=None):
        """
        Execute repairbid silently for automation/manual recovery.

        value_fbc: float or None.  When None the node brute-forces granularities.
        Returns: {'success': bool, 'repaired': int, 'error': str}
        """
        try:
            extra = [str(value_fbc)] if value_fbc is not None else []
            cmd, _ = self.get_fbdctl_command(
                "--wallet", wallet, "repairbid", name, *extra
            )
            result = subprocess.run(
                cmd,
                capture_output=True,
            ...
            if ok:
                if not isinstance(data, dict):
                    data = {}
                return {"success": True, "repaired": data.get("repaired", 0)}
            return {"success": False, "error": err}
        except Exception as e:
            return {"success": False, "error": str(e)}
    ...
    def scan_wallet_auctions(self, wallet):
        ...
        try:
            ...
            # Use listtransactions + covenant parsing for bid discovery:
            # avoids probing `getwalletbids` (RPC -32601) and relies on
            # per-name wallet bid checks instead.
            ...
            for name in found_names:
                try:
                    ...
                    is_automated = (
                        ...
                        not in ["registered", "failed", "lost"]
                        and not existing_job.get("watch_only", False)
                    )
                    is_watched = existing_job is not None and existing_job.get(
                        "watch_only", False
                    )
                    if is_automated:
                        self.log(
                    ...
                    if state not in ["OPENING", "BIDDING", "REVEAL", "CLOSED"]:
                        self.log(
                            f"  Skipping '{name}' - state is {state}, not auction-related"
                        )
                        continue
                    ...
                    tag = (
                        " [automated]"
                        if is_automated
                        else (" [watched]" if is_watched else " [untracked]")
                    )
                    self.log(
            ...
            try:
                ...
                if existing_job and existing_job.get("status") not in [
                    ...
                    messagebox.showwarning(
                        ...
                        f"'{name}' is already in the automation list for wallet '{wallet}'.\n\n"
    ...
    def show_import_auctions_dialog(self, auctions):
        ...
        ttk.Label(legend_frame, text="Legend:", font=("Arial", 9, "bold")).pack(
            side="left"
        )
        ttk.Label(
            legend_frame,
        ...
        for auction in auctions:
            ...
            if is_managed:
                ...
                row_tag = (
                    "importable" if recommendation.get("importable") else "untracked"
                )
        ...
        default_sel = [
            ...
            if not auction_items[iid]["is_managed"]
            and auction_items[iid]["recommendation"].get("importable")
        ]
        ...
        def import_selected():
            ...
            if not selected:
                messagebox.showwarning(
                    "No Selection", "Please select at least one item to import."
                )
                return
        ...
        def track_selected():
            ...
            if not selected:
                messagebox.showwarning(
                    "No Selection", "Please select at least one item to track."
                )
                return
        ...
        self._create_button(
            button_frame, text="Select All (visible)", command=select_visible
        ).pack(side="left", padx=5)
        self._create_button(
            button_frame, text="Deselect All", command=deselect_all
        ).pack(side="left", padx=5)
        self._create_button(
            button_frame, text="Track Activity (Manual)", command=track_selected
        ).pack(side="right", padx=5)
        self._create_button(
            button_frame, text="Import as Job", command=import_selected
        ).pack(side="right", padx=5)
        self._create_button(button_frame, text="Cancel", command=close_dialog).pack(
            side="right", padx=5
        )
    ...
    def _get_import_recommendation(self, auction):
        ...
        if state == "OPENING":
            return {
                ...
                "details": watch_prefix
                + "OPEN tx submitted. Will auto-bid when BIDDING starts (set bid amount after import).",
                "importable": True,
                "manual_required": False,
            }
        ...
        if state == "BIDDING":
            if len(bids) > 0:
                return {
                    ...
                    "details": watch_prefix
                    + "Bid already placed. Will auto-reveal when REVEAL phase starts.",
                    "importable": True,
                    "manual_required": False,
                }
            return {
            ...
            if len(bids) > 0:
                ...
                if unrevealed:
                    return {
                        ...
                        "details": watch_prefix
                        + f"{len(unrevealed)} bid(s) need revealing. Will reveal now.",
                        "importable": True,
                        "manual_required": False,
                    }
                return {
                    ...
                    "details": watch_prefix
                    + "Bids revealed. Will auto-register if won.",
                    "importable": True,
                    "manual_required": False,
                }
            return {
                ...
                if len(bids) > 0:
                    ...
                    if revealed_bids:
                        return {
                            ...
                            "details": watch_prefix
                            + "Auction lost. Will redeem locked funds now.",
                            "importable": True,
                            "manual_required": False,
                        }
                return {
    ...
    def _execute_import(self, auction, recommendation):
        ...
        try:
            ...
            if not recommendation.get("importable", True) and (
                ...
                self.log(
                    f"[!] Skipping '{name}' (not importable): {recommendation['details']}"
                )
                return False
    ...
    def _add_imported_job(self, name, wallet, bids, status):
        ...
        self.log(
            f"[*][YEN] Imported job: {name} (ID: {job_id[:8]}..., Status: {status})"
        )
    ...
    def update_job_runtime_fields(self, job_id, **kwargs):
        """Persist runtime-only job fields such as retry_count without changing status."""
        try:
            jobs_data = self.load_auction_jobs()

            job = None
            for candidate in jobs_data["jobs"]:
                if candidate["id"] == job_id:
                    job = candidate
                    break

            if not job:
                self.log(f"[!] Job not found for runtime update: {job_id[:8]}...")
                return False
            ...
            job["updated_at"] = datetime.now().isoformat()
            return self.save_auction_jobs(jobs_data)
        except Exception as e:
            self.log(f"Error updating runtime fields for job {job_id[:8]}...: {e}")
            return False
...
def main():
    ...
    root._ui_toolkit_mode = (
        toolkit_mode if toolkit_mode in SUPPORTED_UI_TOOLKITS else UI_TOOLKIT_TTK
    )
    app = FBDManager(root)
