...
class AuctionMonitor:
    ...
    def _execute_reveal(self, job):
        ...
        try:
            ...
            self.manager.log(
                f"[OK]- Error executing auto-reveal for '{job['name']}': {e}"
            )
            # Stage 4: Notify error
            self.manager.notification_manager.notify_failed(
                job["name"], job["id"], str(e)
...
class FBDManager:
    ...
    def create_auction_tab(self):
        ...
        self.names_tree.column("Name", width=180, anchor="w")
        ...
        ttk.Label(
            mynames_frame,
            text=(
                "Pricing note: no free names; if no one else bids you still pay minimum bid. "
                "Estimated renewal cost is ~1% of the price paid."
            ),
            font=("Arial", 8, "italic"),
            foreground="#666",
            wraplength=760,
    ...
    def refresh_jobs_list(self):
        ...
        try:
            ...
            self.jobs_tree.tag_configure("manual_watch", foreground="#6688aa")
    ...
    def _get_job_progress_text(self, job, snapshot=None):
        ...
        if status == "pending_open":
            ...
            chain_state = job.get("chain_state", "?")
            return f"[!] Manual action required (chain state: {chain_state})"
        else:
            return status
            ...
            if result.returncode == 0:
                ...
                if not names or len(names) == 0:
                    ...
                    for name_data in names:
                        ...
                        if stats.get("daysUntilExpire") is not None:
                            try:
                                renew_due_text = (
                                    f"{int(float(stats.get('daysUntilExpire')))} days"
                                )
                            except (TypeError, ValueError):
                                renew_due_text = str(stats.get("daysUntilExpire"))
                        elif stats.get("blocksUntilExpire") is not None:
                            try:
                                blocks = int(stats.get("blocksUntilExpire"))
                                renew_due_text = (
                                    f"{max(0, (blocks * 2) // (60 * 24))} days"
                                )
                            except (TypeError, ValueError):
                                renew_due_text = (
                                    f"~{stats.get('blocksUntilExpire')} blks"
                                )
                        else:
                            renewal_block = (
                ...
                if (
                    ...
                    messagebox.showerror(
                        ...
                        "Use Wallet tab dropdown to select an existing wallet or create one.",
                    )
                else:
                    self.log(f"Error loading names: {error_msg}")
                    messagebox.showerror("Error", f"Failed to load names:\n{error_msg}")
    ...
    def _make_treeview_sortable(self, tree, columns):
        """Bind column heading clicks to sortable handlers."""
        ...
        tree_id = id(tree)
        for col in columns:
            label = tree.heading(col).get("text", col)
    ...
    def _sort_treeview_column(self, tree, col):
        """Sort treeview by selected column and toggle direction."""
        import re as _re
        ...
        if col == "Created":
            items.sort(
                key=lambda item: (
                    _parse_relative_age_seconds(item[0])
                    if _parse_relative_age_seconds(item[0]) is not None
                    else float("inf")
                ),
                reverse=reverse,
            )
            ...
            def _num_key(s):
                match = _re.search(r"\d+(?:\.\d*)?", (s or "").replace(",", ""))
                return float(match.group()) if match else None

            numeric_values = [_num_key(value) for value, _ in items]
            if any(v is not None for v in numeric_values):
                items.sort(
                    key=lambda item: (
                        _num_key(item[0])
                        if _num_key(item[0]) is not None
                        else float("-inf")
                    ),
                    reverse=reverse,
                )
            else:
                items.sort(key=lambda item: (item[0] or "").lower(), reverse=reverse)

        for index, (_, iid) in enumerate(items):
            tree.move(iid, "", index)

        labels = getattr(self, "_tree_col_labels", {})
        for heading_col in tree["columns"]:
            base = labels.get((tree_id, heading_col), heading_col)
            indicator = (" ▼" if reverse else " ▲") if heading_col == col else ""
            tree.heading(heading_col, text=base + indicator)
...
"""
        ...
        try:
            ...
            for name in found_names:
                try:
                    ...
                    # Include OPENING (open tx sent) plus all active auction states
    ...
    def import_wallet_auctions(self):
        """
        Scan ALL wallets in the node for auction activity (node-wide view).
        Shows all names with wallet attribution, allowing user to aggregate across wallets.
        """
        if not self.check_node_running():
            response = messagebox.askyesno(
                "Node Not Running",
        ...
        self.log("Scanning ALL wallets for auction activity...")
        try:
            ...
            # Scan all wallets and aggregate results
            all_auctions = []
            ...
            self.log(f"")
            self.log(f"Total: Found {len(all_auctions)} item(s) across all wallets")
    ...
    def show_import_auctions_dialog(self, auctions):
        """
        Display ALL wallet activity found during node-wide scan.
        Shows wallet attribution for each name.
        Sortable columns. Filterable (hide already imported).
        """
        dialog = self._create_toplevel()
        ...
        header = ttk.Label(
            dialog,
            text=f"Node-wide Auction Activity — {len(auctions)} item(s) found",
            font=("Arial", 12, "bold"),
        )
        ...
        ttk.Label(
            legend_frame,
            text="  [AUTO] = already in automation",
            foreground="#888888",
            font=("Arial", 9),
        ).pack(side="left")
        ttk.Label(
        ...
        tree.column("Bids", width=50)
        tree.column("Wallet", width=130)
        tree.column("Tracking", width=120)
        tree.column("Action", width=200)
        tree.column("Details", width=340)
        ...
        for auction in auctions:
            ...
            item_id = tree.insert(
                ...
                values=(
                    ...
                    auction["state"],
                    len(auction["bids"]),
                    wallet[:20],  # Truncate long wallet names
                    tracking_text,
        ...
        def sort_by_column(col):
            ...
            if col == "Bids":
                items = sorted(
                    items,
                    key=lambda x: int(tree.set(x, col)),
                    reverse=sort_state["reverse"],
                )
            else:
                items = sorted(
                    items, key=lambda x: tree.set(x, col), reverse=sort_state["reverse"]
                )
        ...
        def apply_filter():
            ...
            # Restores detached rows when unchecked and keeps table ordering stable.
            tree.set_children("", *visible_ids)

        hide_imported_check.config(command=apply_filter)

        # Default selection: non-automated importable items
        default_sel = [
            iid
        ...
        def select_unmanaged():
            visible = [iid for iid in tree.get_children()]
        ...
        def import_selected():
            """Import selected items as automation jobs."""
            selected = tree.selection()
            if not selected:
                messagebox.showwarning(
        ...
        def track_selected():
            """Track selected items as manual watch entries."""
            selected = tree.selection()
            if not selected:
                messagebox.showwarning(
            ...
            msg = (
                ...
                "These appear in the Jobs list as '[~] Tracked (Manual)'.\n\n"
                "[!] Manual action will be required."
            )
            if skipped_count:
    ...
    def _get_import_recommendation(self, auction):
        ...
        # Already fully automated by an app job
        ...
        watch_prefix = ""
        # Manual watch entries should still produce executable recommendations so they can be upgraded.
        if auction.get("is_watched"):
            watch_prefix = "Tracked manually. Upgrade available. "

        # OPENING: wallet sent OPEN tx, bidding phase not yet started
        if state == "OPENING":
            return {
        ...
        if state == "BIDDING":
            if len(bids) > 0:
                ...
                # No bid yet - can't auto-add without bid amount; manual action needed
                return {
            ...
            if len(bids) > 0:
                ...
                # No bids in REVEAL - wallet has activity but may need manual reveal
                return {
            ...
            if owner:
                ...
                if can_register:
                    return {
                        ...
                        "details": watch_prefix
                        + "Auction won! Will register name now.",
                        "importable": True,
                        "manual_required": False,
                    }
                elif len(bids) > 0:
        ...
        # Fallback for any other state with wallet activity
        return {
            "action": "[~] Activity only",
        ...
        try:
            ...
            if "Execute REVEAL immediately" in action:
                ...
                if "REVEAL pending" in action:
                    ...
                    # OPENING state: created with auto_bid disabled until user sets bid amount
                    status = "opened"
                else:
                    status = "opened"
    ...
    def _add_activity_watch(self, name, wallet, bids, state):
        ...
        Shows in the Jobs list as '[~] Tracked (Manual)' with a reminder
        that manual action is required for any auction steps.
        """
        # Avoid duplicates
        ...
        # Extract bid/lockup from wallet bids if present
        bid_amount = 0.0
        lockup_amount = 0.0
        if bids:
            for bid in bids:
    ...
    def get_job_by_name(self, name, wallet=None):
        ...
        If wallet is None, returns any active job with that name (legacy behavior).
        If wallet is specified, returns only active jobs with that name+wallet combo.
        ...
        try:
            ...
            for job in jobs_data["jobs"]:
                if job["name"] == name and job["status"] not in [
                    ...
                    # If wallet filter specified, check it matches
